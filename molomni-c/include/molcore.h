// Minimal C header for molomni C FFI - concept validation only
#ifndef MOLOMNI_H
#define MOLOMNI_H
...
// zero-copy in-place operations on raw pointers (size must be >= 3)
mc_status_t mc_vec3f32_scale_inplace(float* ptr, float alpha, uintptr_t len);
mc_status_t mc_vec3f32_add_inplace(float* a, const float* b, uintptr_t len);
mc_status_t mc_vec3f64_scale_inplace(double* ptr, double alpha, uintptr_t len);
mc_status_t mc_vec3f64_add_inplace(double* a, const double* b, uintptr_t len);
