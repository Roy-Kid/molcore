...
HEADER = os.path.relpath(os.path.join(ROOT, "molomni-c", "include", "molcore.h"))
...
class AstVisitor(c_ast.NodeVisitor):
    ...
    def visit_Decl(self, node):
        if not node.name or not node.name.startswith("mc_"):
            return
        # functions only
        if isinstance(node.type, c_ast.FuncDecl):
            ...
            if node.type.args is not None:
                for parameter in node.type.args.params:
                    if isinstance(parameter.type, c_ast.TypeDecl) and parameter.name is None and isinstance(parameter.type.type, c_ast.IdentifierType) and parameter.type.type.names == ['void']:
                        # void parameter list -> no args
                        continue
...
def funcdecl_to_ctypes(type):
    ...
    args = [type_to_ctypes(t.type) for t in (type.args.params or [])]
    # functions taking void parameter in C don't have any parameter
    args = [a for a in args if a != "None"]
    return f"CFUNCTYPE({restype}, {', '.join(args)})" if args else f"CFUNCTYPE({restype})"
